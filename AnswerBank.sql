...
temp2_cte as (
    select
		customer_id,
		DATEDIFF(order_date, prev_order_date) AS order_gap
    From
		Temp_cte
    )
SELECT
    customer_id,
    MAX(order_gap) AS longest_gap
FROM temp2_cte
GROUP BY customer_id;

-- B. Find the difference in days between the latest and second latest purchase
...
SELECT
    t1.customer_id,
    DATEDIFF(t1.order_date, t2.order_date) AS gap
FROM temp_cte t1
JOIN temp_cte t2
ON t1.customer_id = t2.customer_id
WHERE t1.rnk = 1
...
Select
	product_id,
    month,
    lag(sales, 2) over(PARTITION BY product_id order by month) as two_month_rev_Sales,
...
Select product_id
From Temp_CTE
where two_month_rev_Sales > prev_sales and prev_sales > current_month_Sales;
...
From
    ...
    Select 
		User_id,
        timestampdiff(minute, session_end, session_start) as session_duration
	From Sessions
    ) t
group by User_id;

# method 2: without using subquery
SELECT
    User_Id,
...
Select quarter, Salesperson_id, total_revenue
From
	(
    select 
		quarter, 
        ...
        dense_rank () over(partition by quarter order by SUM(revenue) DESC) as rnk
	From
		sales
	GROUP BY quarter, salesperson_id
    ) t
where rnk = 1;
...
where datediff(U.signup_date, A.Activity_date) > 30;
...
select
	month,
    round((current_revenue - prev_revenue)/prev_revenue *100,2) as MoM_revenue
From
	(
    Select
		month,
...
(Select
	customer_id,
    round(count(R.order_id)/count(O.order_id) *100,2) as return_per
From ORders O
left join Returns R
on o.order_id = R.order_id
group by customer_id)
...
select customer_id
from temp_cte
where return_per in ( select max(return_per) from temp_cte);
...
SELECT
    ROUND(SUM(amount)/COUNT(DISTINCT u.user_id),2) AS ARPU
...
from
	...
	select
		s.region,
		p.category,
		dense_rank () over (partition by region order by sum(s.quantity) desc) as rnk
	from Sales S
	JOin products p
...
Select
	round(count(*)/(select count(distinct customer_id) from orders)*100,2) as return_per
from
...
# Method 2: without using subquery.  
	-- True cohort analysis is needed, usually both are shown:
SELECT
...
AND t.feature_name = f.feature_name
AND f.usage_date > t.first_usage_date
...
SELECT
    stage_name,
    users_count,
    ROUND(users_count/LAG(users_count) OVER(ORDER BY users_count DESC)* 100, 2) AS conversion_percentage
FROM temp_cte;

## NOTE: If funnel order exists explicitly (stage sequence), use that instead of: ORDER BY users_count DESC
...
From 
    ...
    select 
		C.Region,
        C.customer_id,
        dense_rank () over(partition by C.region order by sum(O.revenue) DESC) as rnk
	From Customers C
    Join orders O
    on C.customer_id = O.customer_id
...
-- Identify unusual revenue spikes or drops.
	...
	-- Considering more than 50% unusual
...
Select
	Sale_Date,
    Revenue as current_Revenue,
    lag(revenue) over(order by Sale_date) as prev_month_revenue
From
	Daily_revenue
)
...
From Temp_CTE
Group by sale_date
having (current_revenue - prev_month_revenue)/current_revenue > 0.5;
...
From logins
group by user_id
having count(distinct devide_id) > 1;
...
-- Analyze repeat purchases category-wise.
	-- Need customers purchasing same category more than once
...
Select
	customer_id,
    avg(datediff(order_date,prev_order_date)) as avg_time_gap
from
	(
    select 
		customer_id, 
...
(Select 
	Product_id,
    round((sales - prev_month_sales)/prev_month_sales*100,2) as MoM_Sales
From Temp_CTE
...
Select
	product_id
from temp2_cte
where Mom_sales in (select max(Mom_sales) from temp2_cte);
...
(
    SELECT
        product_id,
        ROUND ((sales - LAG(sales) OVER(PARTITION BY product_id ORDER BY month))/
            LAG(sales) OVER(PARTITION BY product_id ORDER BY month) * 100,2 ) AS growth_rate
    FROM monthly_sales
)

SELECT product_id
FROM temp_cte
WHERE growth_rate =(SELECT MAX(growth_rate)FROM temp_cte);
...
Select
	sale_date,
    sum(revenue) over(order by sale_date rows between 29 preceding and current row) as rolling_30_day_revenue
from daily_sales;
